...
import { CalendarIcon, Package, AlertCircle, Eye, FileSignature, ShoppingCart, Loader2, Search } from 'lucide-react';
...
export default function RemittedStocksPage() {
  const { user } = useAuth();
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [remittances, setRemittances] = useState<RemittanceLog[]>([]);
  const [filteredRemittances, setFilteredRemittances] = useState<RemittanceLog[]>([]);
  ...
  useEffect(() => {
    ...
    const channels = [
      subscribeToTable('remittances_log', () => {
        console.log('🔄 Real-time: Remittance log updated');
        fetchAllRemittances();
      }),
  ...
  const fetchAllRemittances = async () => {
    ...
    setLoading(true);
    try {
      // Build query for ALL remittances (admin view)
      let query = supabase
      ...
      setFilteredRemittances(formattedData);
    } catch (error: any) {
  ...
  const totalRemittances = filteredRemittances.length;
  const totalItems = filteredRemittances.reduce((sum, r) => sum + r.items_remitted, 0);
  const totalUnits = filteredRemittances.reduce((sum, r) => sum + r.total_units, 0);
  const totalRevenue = filteredRemittances.reduce((sum, r) => sum + r.total_revenue, 0);
  const totalOrders = filteredRemittances.reduce((sum, r) => sum + r.orders_count, 0);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold">All Remittances</h1>
          <p className="text-muted-foreground mt-1">
            View remittances from all agents across the organization
          </p>
        </div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-[180px] justify-start text-left font-normal">
                ...
                {selectedDate ? format(selectedDate, 'MMM dd, yyyy') : <span>Filter date</span>}
              </Button>
            </PopoverTrigger>
      ...
      <Card>
        <CardHeader>
          <CardTitle>All Agent Remittances</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-muted rounded-lg">
                ...
                <div>
                  ...
                  <p className="font-semibold">{selectedRemittance.leader_name}</p>
                </div>
                <div>
                ...
                <TabsContent value="summary" className="space-y-4">
                  ...
                  <div className="border rounded-lg p-4 bg-blue-50 border-blue-200">
                    ...
                    <ul className="text-sm space-y-1 text-blue-800">
                      <li>✓ Agent: <strong>{selectedRemittance.agent_name}</strong> remitted to Leader: <strong>{selectedRemittance.leader_name}</strong></li>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-green-700 mt-1">
                              ...
                              It confirms <strong>{selectedRemittance.agent_name}'s</strong> acknowledgment and authorization of this transaction to <strong>{selectedRemittance.leader_name}</strong>.
                            </p>
                          </div>
                        </div>
                      </div>
