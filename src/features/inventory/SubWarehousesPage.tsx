...
export default function SubWarehousesPage() {
  ...
  const getReturnLotTypeGroups = (rows: ReturnLotRow[]): [string, ReturnLotRow[]][] => {
    const m = new Map<string, ReturnLotRow[]>();
    for (const r of rows) {
    ...
    return Array.from(m.entries())
      .map(([type, list]) =>
        [type, list.sort((a, b) => a.variantName.localeCompare(b.variantName) || a.batch_number.localeCompare(b.batch_number))] as [
          string,
          ReturnLotRow[],
        ]
      )
  ...
  const returnFromLocation = async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (data && (data as any).success === false) throw new Error((data as any).error || 'Return request failed');
      ...
      await qc.invalidateQueries({ queryKey: ['warehouse-return-batch-lots', returnLocationId] });
      await qc.invalidateQueries({ queryKey: ['variant-batch-lots'] });
      await qc.invalidateQueries({ queryKey: ['warehouse-stock-returns'] });
      await qc.refetchQueries({ queryKey: ['warehouse-stock-returns'] });
    } catch (e: any) {
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 min-h-[200px] max-h-[55vh] border rounded-md p-3 bg-muted/20">
          {!returnLocationId ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-8">No batch lots at this sub-warehouse.</p>
