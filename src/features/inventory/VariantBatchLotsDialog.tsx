...
import { fetchReceivePackingByLotIds } from './utils/formatReceivePacking';
...
export function VariantBatchLotsDialog({
  ...
  const {
    ...
    queryFn: async (): Promise<VariantBatchLotRow[]> => {
      ...
      const packingByLotId = await fetchReceivePackingByLotIds(mapped.map((lot) => lot.lot_id));
      return mapped.map((lot) => ({
        ...lot,
        packing: packingByLotId[lot.lot_id] ?? null,
      }));
    },
  });
