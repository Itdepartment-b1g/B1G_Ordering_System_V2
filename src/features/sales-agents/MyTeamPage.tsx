...
export default function MyTeamPage() {
  ...
  const [assignedHubNames, setAssignedHubNames] = useState<string[]>([]);
  const [loadingAssignedHub, setLoadingAssignedHub] = useState(true);
    ...
    try {
      ...
      const teamMembersWithSales = teamData
        .map((member: any) => {
          ...
          const approvedOrders = salesData.filter(
            (order: any) => order.status === "approved",
          ).length;
          const pendingOrders = salesData.filter(
            (order: any) => order.status === "pending",
          ).length;
          const rejectedOrders = salesData.filter(
  ...
  useEffect(() => {
    let cancelled = false;
    const loadAssignedHub = async () => {
      if (!user || user.role !== "team_leader") {
        setAssignedHubNames([]);
        setLoadingAssignedHub(false);
        return;
      }
      setLoadingAssignedHub(true);
      try {
        ...
        const names = (data ?? [])
          .map((row: { hub_name: string }) => row.hub_name)
          .filter(Boolean);
        setAssignedHubNames(names);
      } catch (e) {
        console.error("Error loading assigned hub:", e);
        if (!cancelled) setAssignedHubNames([]);
      } finally {
        if (!cancelled) setLoadingAssignedHub(false);
      }
    };
    void loadAssignedHub();
    return () => {
      cancelled = true;
    };
  }, [user]);
  ...
  const totalTeamOrders = teamMembers.reduce(
    (sum, member) => sum + member.ordersCount,
    0,
  );
          ...
          <CardContent className="p-12">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Only leaders can access this page. Please contact your
          ...
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center justify-between">
              <div className="min-w-0 flex-1">
                <p className="text-[10px] md:text-sm font-medium text-muted-foreground mb-1 truncate">
                  Total Members
                </p>
            ...
            <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
              ...
              <div className="flex gap-2">
                <select
                  value={sortBy}
                  onChange={(e) =>
                    setSortBy(e.target.value as "name" | "sales" | "orders")
                  }
                  ...
                  <div className="flex justify-center gap-2 text-[10px] text-muted-foreground">
                    ...
                    {member.pendingOrders > 0 && (
                      <span className="text-orange-600">
                        {member.pendingOrders} pending
                      </span>
                    )}
                    {member.rejectedOrders > 0 && (
                      <span className="text-red-600">
                        {member.rejectedOrders} rejected
                      </span>
                    )}
                  </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="border rounded-lg p-3 bg-background">
                          ...
                          <p className="text-xs mt-1">
                            {selectedMember.region || "Not provided"}
                          </p>
                        </div>
                        <div className="border rounded-lg p-3 bg-background">
                          <Label className="text-xs font-medium text-muted-foreground">
