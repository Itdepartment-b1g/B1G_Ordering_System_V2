...
function formatPhp(amount: number): string {
  return `Γé▒${amount.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
}
...
function groupItemsByBrand(items: KeyAccountPoPaymentReminderEmailItem[]) {
  ...
  for (const it of items) {
    const brandName = String(it.brandName || '').trim() || 'ΓÇö';
...
export function generateKeyAccountPoPaymentReminderHTML(
  ...
  const clientName = escapeHtml(String(data.clientName || 'ΓÇö'));
  const paymentTerms = escapeHtml(String(data.paymentTerms || 'ΓÇö'));
  const notificationDate = escapeHtml(String(data.notificationDate || 'ΓÇö'));
  const poViewUrl =
      ...
      ? brandGroups
          .map((group, groupIndex) => {
            ...
            const variantRows = group.variants
              .map((it, index) => {
                const variant = escapeHtml(String(it.variantName || '').trim() || 'ΓÇö');
