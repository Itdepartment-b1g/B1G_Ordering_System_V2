-- ============================================================================
-- CREATE CLIENT ORDER ATOMIC (standalone deploy script)
-- Same as migration 20260811120000_create_client_order_atomic.sql
-- ============================================================================
...
BEGIN
  ...
  INSERT INTO client_orders (
    ...
    order_date, subtotal, tax_rate, tax_amount, discount, total_amount,
    notes, signature_url, payment_method, bank_type, payment_proof_url,
    payment_mode, payment_splits, status, stage, remitted, pricing_strategy
  ) VALUES (
    v_company_id, v_order_number, p_agent_id, p_client_id, v_client_account_type,
    p_order_date, COALESCE(p_subtotal, 0), 0, COALESCE(p_tax_amount, 0),
    COALESCE(p_discount, 0), COALESCE(p_total_amount, 0),
    p_notes, p_signature_url, p_payment_method, p_bank_type, p_payment_proof_url,
    COALESCE(p_payment_mode, 'FULL'), p_payment_splits, 'pending',
    COALESCE(p_stage, 'agent_pending'), COALESCE(p_remitted, FALSE),
  ...
  LOOP
    INSERT INTO client_order_items (
      ...
      v_item.unit_price, v_item.selling_price, v_item.dsp_price, v_item.rsp_price,
