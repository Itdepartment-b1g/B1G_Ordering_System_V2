-- Fix approve_warehouse_transfer_po stock source:
-- - If requested location is the Main Warehouse location, validate/deduct from main_inventory (available = stock - allocated_stock)
-- - If requested location is a Sub-warehouse, validate/deduct from warehouse_location_inventory and decrement main_inventory.allocated_stock
...
BEGIN
  ...
  SELECT wl.is_main INTO v_is_main_location
  FROM public.warehouse_locations wl
  WHERE wl.id = po_record.warehouse_location_id;

  v_is_main_location := COALESCE(v_is_main_location, false);

  -- Validate stock first (avoid partial approvals)
  FOR item_record IN
  ...
  LOOP
    IF v_is_main_location THEN
      SELECT (mi.stock - COALESCE(mi.allocated_stock, 0)) INTO main_available
      FROM public.main_inventory mi
      WHERE mi.company_id = po_record.warehouse_company_id
        AND mi.variant_id = item_record.variant_id;

      IF main_available IS NULL THEN
        RETURN json_build_object('success', false, 'error', 'Variant not stocked at main warehouse: ' || item_record.variant_name);
      END IF;

      IF main_available < item_record.quantity THEN
        RETURN json_build_object('success', false, 'error', 'Insufficient main warehouse available stock for variant ' || item_record.variant_name);
      END IF;
    ELSE
      ...
      IF NOT FOUND THEN
        RETURN json_build_object('success', false, 'error', 'Variant not stocked at sub-warehouse: ' || item_record.variant_name);
      END IF;

      IF loc_stock < item_record.quantity THEN
        RETURN json_build_object('success', false, 'error', 'Insufficient sub-warehouse stock for variant ' || item_record.variant_name);
      END IF;
    END IF;
  END LOOP;
  ...
  LOOP
    IF v_is_main_location THEN
      -- Deduct from main inventory stock (available stock decreases)
      ...
      INSERT INTO inventory_transactions (
        ...
        'Warehouse transfer out PO ' || po_record.po_number || ' from main warehouse',
        NOW()
      );
    ELSE
    ...
    IF v_client_variant_id IS NULL THEN
      ...
      IF v_client_brand_id IS NULL THEN
        ...
        VALUES (item_record.client_company_id, item_record.brand_name, NULL, approver_id, NOW(), NOW())
        RETURNING id INTO v_client_brand_id;
      END IF;

      -- Find client variant_type_id (case-insensitive match)
      ...
      IF v_client_variant_id IS NULL THEN
        INSERT INTO public.variants (
          company_id, brand_id, variant_type_id, name, variant_type, description, sku, created_at, updated_at
        ) VALUES (
          item_record.client_company_id, v_client_brand_id, v_client_variant_type_id,
          item_record.variant_name, item_record.variant_type, NULL, NULL, NOW(), NOW()
        )
      ...
      INSERT INTO public.warehouse_variant_mappings (
        ...
        item_record.client_company_id, po_record.warehouse_company_id, item_record.variant_id, v_client_variant_id
      )
    ...
    -- Client-side inventory update (in) using client_variant_id
    ...
    ELSE
      INSERT INTO public.main_inventory (
        ...
        item_record.client_company_id, v_client_variant_id, item_record.quantity, 0, 10, NOW(), NOW()
      );
    END IF;
...
COMMENT ON FUNCTION public.approve_warehouse_transfer_po(uuid, uuid) IS
  'Approves internal PO: if location is main, deduct from main_inventory (stock - allocated_stock); if sub, deduct from location inventory and decrement allocated_stock. Then increment client inventory via mapping.';
