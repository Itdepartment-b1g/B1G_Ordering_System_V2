-- Key Account warehouse_transfer POs: fulfill only deducts hub stock.
-- Do not require warehouse_variant_mappings, client variant_types, or credit buyer main_inventory
-- (standard transfer flow assumes a tenant catalog + inventory on company_id).
...
COMMENT ON FUNCTION public.fulfill_po_location(uuid, uuid, uuid) IS
  'Fulfills warehouse_transfer reservations: deducts hub stock. Standard Accounts: maps variants and credits buyer main_inventory. Key Accounts: hub deduction + transactions only; no client catalog or buyer inventory.';
