-- Undo: restores fulfill_po_location to the behavior from 20260413170000
-- (always maps variants + credits buyer main_inventory).
-- Apply this if you need to roll back 20260515100000_fulfill_po_location_key_accounts_skip_client_inventory.sql.
-- Warning: Key Account warehouse transfers will again require client variant_types / mappings.
...
COMMENT ON FUNCTION public.fulfill_po_location(uuid, uuid, uuid) IS
  'Reverted to pre–Key Account branch: always credits buyer main_inventory and requires client variant mapping.';
