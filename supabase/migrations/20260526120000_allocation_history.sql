-- Allocation history: session header + line items via inventory_transactions.reference_*

-- ---------------------------------------------------------------------------
-- 1) allocation_history (header only)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.allocation_history (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies (id) ON DELETE CASCADE,
  allocated_to uuid NOT NULL REFERENCES public.profiles (id) ON DELETE CASCADE,
  allocated_by uuid NOT NULL REFERENCES public.profiles (id) ON DELETE RESTRICT,
  brand_id uuid REFERENCES public.brands (id) ON DELETE SET NULL,
  allocation_type text NOT NULL DEFAULT 'main_to_leader',
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT allocation_history_allocation_type_check CHECK (
    allocation_type = ANY (ARRAY['main_to_leader'::text, 'leader_to_agent'::text])
  )
);

COMMENT ON TABLE public.allocation_history IS 'Grouped stock allocation sessions.';
COMMENT ON COLUMN public.allocation_history.allocated_to IS 'Profile id of the user who received stock (team leader or agent).';
COMMENT ON COLUMN public.allocation_history.allocated_by IS 'Profile id of the user who performed the allocation.';
COMMENT ON COLUMN public.allocation_history.allocation_type IS 'main_to_leader | leader_to_agent';

CREATE INDEX IF NOT EXISTS idx_allocation_history_company_created
  ON public.allocation_history (company_id, created_at DESC);
...
-- ---------------------------------------------------------------------------
-- 3) allocate_to_leader — optional link to allocation_history on inventory_transactions
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO inventory_transactions (
    ...
    v_company_id, p_variant_id, 'allocated_to_agent', p_quantity,
    'main_inventory', CONCAT('agent_inventory:', p_leader_id), p_performed_by,
