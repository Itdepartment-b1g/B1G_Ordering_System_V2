-- Batch-approve pending stock requests from one agent in a single allocation_history session.
-- Uses allocate_batch_to_agent for inventory moves + linked inventory_transactions.
...
GRANT EXECUTE ON FUNCTION public.approve_stock_requests_batch_by_leader(uuid[], uuid, text) TO authenticated;
