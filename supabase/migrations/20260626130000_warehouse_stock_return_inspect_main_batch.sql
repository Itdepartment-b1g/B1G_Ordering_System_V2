-- Stock return inspect: main warehouse picks destination batch lot (not sub-warehouse source).

-- ---------------------------------------------------------------------------
-- 1) Receipt lines: destination lot at main
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2) LIFO deduct at a location (sub-warehouse batch cleanup on return)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) receive_warehouse_stock_return_request — destination_lot_id at main
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    IF v_item.lot_id IS NOT NULL THEN
      ...
      IF NOT FOUND THEN
        RETURN json_build_object(
          'success', false,
          'error', 'Legacy return batch lot not found at sub-warehouse',
          'request_item_id', v_request_item_id
        );
      END IF;
      ...
      IF v_source_lot.quantity_remaining < (v_qty_good + v_qty_damaged) THEN
        RETURN json_build_object(
          'success', false,
          'error', 'Insufficient quantity in legacy return batch lot',
          'request_item_id', v_request_item_id,
  ...
  LOOP
    ...
    ELSE
      ...
      INSERT INTO public.warehouse_stock_return_receipt_lines (
        receipt_id, request_item_id, variant_id, qty_good, qty_damaged, destination_lot_id
      ) VALUES (
        v_receipt_id, v_item.id, v_variant_id, v_qty_good, v_qty_damaged, v_destination_lot_id
      );
