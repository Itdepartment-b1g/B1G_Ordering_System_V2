-- Rebate return receive: manual batch lot splits (good/damaged per destination lot).
-- Replaces automatic restore_rebate_return_inventory_lots with user-selected destination lots.

ALTER TABLE public.key_account_po_rebate_return_receipt_lines
  ADD COLUMN IF NOT EXISTS destination_lot_id uuid
  REFERENCES public.inventory_batch_lots(id) ON DELETE SET NULL;

COMMENT ON COLUMN public.key_account_po_rebate_return_receipt_lines.destination_lot_id IS
  'Warehouse batch lot where good-condition qty is restocked. Multiple receipt lines allowed per rebate line.';
...
BEGIN
  ...
  -- Validate each distribution row.
  ...
  LOOP
    ...
    IF v_total_qty <= 0 THEN
      RETURN json_build_object('success', false, 'error', 'Each distribution row must have at least one unit');
    END IF;
    ...
    WHERE ibl.id = v_destination_lot_id
      AND ibl.company_id = po_record.warehouse_company_id
      AND ibl.warehouse_location_id = v_wh_loc
      AND ibl.variant_id = v_rebate_line.variant_id;

    IF NOT FOUND THEN
      RETURN json_build_object(
        'success', false,
        'error', 'Selected warehouse batch lot not found for this product and location',
        'rebate_line_id', v_rebate_line.id
      );
    END IF;
  END LOOP;

  -- Aggregate qty per rebate line must not exceed disputed qty.
  FOR v_agg IN
    SELECT
      (elem->>'rebate_line_id')::uuid AS rebate_line_id,
      SUM(
        COALESCE((elem->>'qty_good')::int, 0) + COALESCE((elem->>'qty_damaged')::int, 0)
      )::integer AS total_qty
    FROM jsonb_array_elements(p_lines) AS elem
    GROUP BY (elem->>'rebate_line_id')::uuid
  LOOP
    SELECT disputed_quantity INTO v_agg_total
    FROM public.key_account_po_rebate_lines
    WHERE id = v_agg.rebate_line_id AND rebate_id = v_rebate.id;

    IF NOT FOUND THEN
      RETURN json_build_object('success', false, 'error', 'Invalid rebate line in aggregate check');
    END IF;

    IF v_agg.total_qty > v_agg_total THEN
      RETURN json_build_object(
        'success', false,
        'error', 'Received quantity exceeds disputed quantity for a line',
        'rebate_line_id', v_agg.rebate_line_id,
        'disputed', v_agg_total,
        'received', v_agg.total_qty
      );
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    INSERT INTO public.key_account_po_rebate_return_receipt_lines (
      receipt_id, rebate_line_id, warehouse_location_id, variant_id,
      qty_good, qty_damaged, destination_lot_id
    ) VALUES (
      v_receipt_id, v_rebate_line.id, v_wh_loc, v_rebate_line.variant_id,
      v_qty_good, v_qty_damaged, v_destination_lot_id
    );
    ...
    IF v_qty_good > 0 THEN
      ...
      INSERT INTO public.inventory_batch_movements (
        ...
        po_record.warehouse_company_id, v_destination_lot_id, v_destination_lot.batch_id,
        v_rebate_line.variant_id, v_wh_loc,
        'rebate_return_in', v_qty_good, 'rebate_return_receipt', v_receipt_id,
        v_wh_loc, auth.uid(),
...
COMMENT ON FUNCTION public.receive_key_account_rebate_returns(uuid, jsonb, text) IS
  'Inspect rebate returns with manual batch lot splits: good qty restocks selected lots; damaged goes to disposal.';
