-- Physical count: record-only submit (no inventory / batch lot changes on variance).
...
COMMENT ON FUNCTION public.submit_physical_count(uuid, uuid, jsonb, text, text, text, uuid) IS
  'Record-only physical count: saves signed session and line snapshots without modifying inventory.';
