-- Physical count: snapshot performer display name (avoids profiles RLS on history joins).

ALTER TABLE public.physical_count_sessions
  ADD COLUMN IF NOT EXISTS performed_by_name text;

UPDATE public.physical_count_sessions pcs
SET performed_by_name = NULLIF(trim(p.full_name), '')
FROM public.profiles p
WHERE p.id = pcs.performed_by
  AND pcs.performed_by IS NOT NULL
  AND (pcs.performed_by_name IS NULL OR pcs.performed_by_name = '');
...
COMMENT ON COLUMN public.physical_count_sessions.performed_by_name IS
  'Snapshot of performer full_name at submit time for audit display across roles.';
