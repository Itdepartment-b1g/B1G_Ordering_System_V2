-- Physical count lines: optional loose-box breakdown for audit.
...
ALTER TABLE public.physical_count_lines
  DROP CONSTRAINT IF EXISTS physical_count_lines_loose_box_count_check;

ALTER TABLE public.physical_count_lines
  ADD CONSTRAINT physical_count_lines_loose_box_count_check
  CHECK (loose_box_count IS NULL OR loose_box_count >= 0);

ALTER TABLE public.physical_count_lines
  DROP CONSTRAINT IF EXISTS physical_count_lines_loose_qty_check;

ALTER TABLE public.physical_count_lines
  ADD CONSTRAINT physical_count_lines_loose_qty_check
  CHECK (loose_qty IS NULL OR loose_qty >= 0);

COMMENT ON COLUMN public.physical_count_lines.loose_box_count IS
  'Number of loose/partial boxes counted for this line (optional audit breakdown).';
COMMENT ON COLUMN public.physical_count_lines.loose_qty IS
  'Units per loose/partial box for this line (optional audit breakdown).';
...
BEGIN
  ...
  LOOP
    ...
    v_loose_box_count := COALESCE((v_line->>'loose_box_count')::integer, 0);
    v_loose_qty := COALESCE((v_line->>'loose_qty')::integer, 0);
    ...
    IF v_loose_box_count < 0 OR v_loose_qty < 0 THEN
      RAISE EXCEPTION 'Loose box_count and loose_qty must be non-negative';
    END IF;

    IF v_physical_qty <> (v_box_count * v_units_per_box) + (v_loose_box_count * v_loose_qty) THEN
      RAISE EXCEPTION 'Physical quantity must equal (box_count × units_per_box) + (loose_box_count × loose_qty)';
    END IF;
    ...
    INSERT INTO public.physical_count_lines (
      ...
      v_box_count, v_units_per_box, v_loose_box_count, v_loose_qty
    );
  END LOOP;
