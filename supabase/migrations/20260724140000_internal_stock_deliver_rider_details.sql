-- Rider details on internal stock deliver (request Deliver + main allocation).
-- Requires rider_name, rider_plate_number, rider_photo_url alongside existing
-- delivery proof photo + e-signature.
...
ALTER TABLE public.internal_stock_requests
  ...
  ADD COLUMN IF NOT EXISTS rider_plate_number text,
  ADD COLUMN IF NOT EXISTS rider_photo_url text,
  ADD COLUMN IF NOT EXISTS rider_photo_path text;

COMMENT ON COLUMN public.internal_stock_requests.rider_name IS
  'Rider / courier name captured on deliver (PO-style).';
COMMENT ON COLUMN public.internal_stock_requests.rider_plate_number IS
  'Rider vehicle plate captured on deliver.';
COMMENT ON COLUMN public.internal_stock_requests.rider_photo_url IS
  'Rider photo URL/data URL captured on deliver.';

-- ---------------------------------------------------------------------------
-- 2) deliver_internal_stock_request — require rider trio
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.deliver_internal_stock_request(uuid, text, text, uuid, text, text);
...
BEGIN
  ...
  INSERT INTO public.internal_stock_request_events (
    request_id, event_type, note, lines, proof_image_url, proof_image_path,
...
GRANT EXECUTE ON FUNCTION public.deliver_internal_stock_request(
  uuid, text, text, uuid, text, text, text, text, text, text
) TO authenticated;

COMMENT ON FUNCTION public.deliver_internal_stock_request(
  uuid, text, text, uuid, text, text, text, text, text, text
) IS
  'Deliver approved internal stock request: reserve main, assign DR, require delivery proof + signature + rider name/plate/photo.';

-- ---------------------------------------------------------------------------
-- 3) create_and_deliver_main_stock_allocation — require rider trio
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.create_and_deliver_main_stock_allocation(
...
GRANT EXECUTE ON FUNCTION public.create_and_deliver_main_stock_allocation(
  uuid, jsonb, text, text, text, text, text, text, text, text, text, uuid
) TO authenticated;

COMMENT ON FUNCTION public.create_and_deliver_main_stock_allocation(
  uuid, jsonb, text, text, text, text, text, text, text, text, text, uuid
) IS
  'Main warehouse create+deliver allocation; requires delivery proof, signature, and rider name/plate/photo.';

-- ---------------------------------------------------------------------------
-- 4) list RPC — include rider fields
-- ---------------------------------------------------------------------------
