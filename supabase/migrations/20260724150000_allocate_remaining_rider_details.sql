-- Require rider name/plate/photo on allocate remaining (partial receive waves).
-- Store rider on the remaining_released event so each wave keeps its own rider.
...
COMMENT ON COLUMN public.internal_stock_request_events.rider_name IS
  'Rider name for delivered / remaining_released waves.';
COMMENT ON COLUMN public.internal_stock_request_events.rider_plate_number IS
  'Rider plate for delivered / remaining_released waves.';
COMMENT ON COLUMN public.internal_stock_request_events.rider_photo_url IS
  'Rider photo URL/data URL for delivered / remaining_released waves.';

DROP FUNCTION IF EXISTS public.allocate_internal_stock_request_remaining(
...
BEGIN
  ...
  -- Keep request-level rider as latest wave rider (detail/DR fallback).
...
COMMENT ON FUNCTION public.allocate_internal_stock_request_remaining(
  ...
  'Unlock remaining short for partial receive; requires proof, signature, and rider name/plate/photo.';

-- List RPC: include rider fields on events
...
-- ---------------------------------------------------------------------------
-- Also persist rider on delivered/main-allocation delivered events (per-wave).
-- Backfill existing delivered events from request-level rider.
-- ---------------------------------------------------------------------------
UPDATE public.internal_stock_request_events e
SET
  rider_name = r.rider_name,
  rider_plate_number = r.rider_plate_number,
  rider_photo_url = r.rider_photo_url,
  rider_photo_path = r.rider_photo_path
FROM public.internal_stock_requests r
WHERE e.request_id = r.id
  AND e.event_type IN ('delivered', 'approved_released')
  AND e.rider_name IS NULL
