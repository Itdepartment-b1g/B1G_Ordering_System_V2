-- Per-wave DR numbers: store DR on delivered / remaining_released events.
-- Allocate remaining generates a new DR for each wave.

ALTER TABLE public.internal_stock_request_events
  ADD COLUMN IF NOT EXISTS dr_number text;

COMMENT ON COLUMN public.internal_stock_request_events.dr_number IS
  'Delivery receipt number for delivered / remaining_released waves.';

-- Backfill first-delivery events from request-level DR.
UPDATE public.internal_stock_request_events e
SET dr_number = r.dr_number
FROM public.internal_stock_requests r
WHERE e.request_id = r.id
  AND e.event_type IN ('delivered', 'approved_released')
  AND e.dr_number IS NULL
  AND r.dr_number IS NOT NULL;

-- ---------------------------------------------------------------------------
-- allocate_internal_stock_request_remaining — generate new DR per wave
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Latest wave rider + DR on the request row (list/detail current DR).
...
COMMENT ON FUNCTION public.allocate_internal_stock_request_remaining(
  ...
  'Unlock remaining short for partial receive; requires proof, signature, rider, and generates a new DR per wave.';

-- ---------------------------------------------------------------------------
-- list RPC: project event.dr_number
-- ---------------------------------------------------------------------------
...
-- Persist DR on delivered events (deliver + main allocation)
