-- After TL receives warehouse-transfer stock into main_inventory, allocate received qty
-- to the assigned team leader (allocated_stock + agent_inventory + allocation_history).
...
BEGIN
  ...
  LOOP
    ...
    IF po_record.company_account_type IS DISTINCT FROM 'Key Accounts' AND v_recv_qty > 0 THEN
      ...
      -- Queue for main_to_leader allocation after all stock credits.
  ...
  -- Allocate received qty to assigned TL (stock already credited → available covers allocate).
...
COMMENT ON FUNCTION public.receive_po_delivery(uuid, jsonb, text, text, text, text, uuid) IS
  'Assigned team leader receives a warehouse-transfer dispatch: credits main_inventory, allocates received qty to the TL (allocation_history), shortfalls open warehouse discrepancy tickets.';
