-- Faster list loads: optional skip of nested events (proof URLs, timelines).
-- Detail / PDF paths still fetch a single request with full events via select.
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      CASE
        WHEN COALESCE(p_include_events, true) THEN COALESCE((
          SELECT jsonb_agg(
            jsonb_build_object(
              'id', e.id,
...
GRANT EXECUTE ON FUNCTION public.list_internal_stock_requests_for_caller(uuid, text, boolean)
  TO authenticated;

-- Keep 2-arg overload working (Postgres may require explicit grant on prior signature).
GRANT EXECUTE ON FUNCTION public.list_internal_stock_requests_for_caller(uuid, text)
  TO authenticated;

COMMENT ON FUNCTION public.list_internal_stock_requests_for_caller(uuid, text, boolean) IS
  'List internal stock requests for caller. Pass p_include_events=false for fast list rows without timeline/proof payloads.';
