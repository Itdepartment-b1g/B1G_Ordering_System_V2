-- Found: missing units return to the dispatcher's inventory.
-- Then they either dispatch again (Incoming, new TDR) or keep the stock
-- (transfer incomplete). Keep is not the same as Write off (lost, no stock back).
...
BEGIN
  FOR r IN
    ...
    WHERE con.conrelid = 'public.tl_stock_request_discrepancies'::regclass
      AND con.contype = 'c'
      AND pg_get_constraintdef(con.oid) ILIKE '%resolved_redeliver%'
  LOOP
    EXECUTE format(
      'ALTER TABLE public.tl_stock_request_discrepancies DROP CONSTRAINT IF EXISTS %I',
      r.conname
    );
  END LOOP;
END $$;
...
ALTER TABLE public.tl_stock_request_discrepancies
  ADD CONSTRAINT tl_stock_request_discrepancies_status_check
  CHECK (status = ANY (ARRAY[
    'open'::text,
    'resolved_redeliver'::text,
    'resolved_found_keep'::text,
...
BEGIN
  ...
  ELSIF v_new_item_status = 'pending_receipt' THEN
    PERFORM public._tl_notify(
      ...
      v_header.requester_leader_id,
      'approved',
      'Replacement dispatched — ready to receive',
