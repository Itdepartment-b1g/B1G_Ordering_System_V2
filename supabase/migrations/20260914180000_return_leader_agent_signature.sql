-- Store agent signature on return-to-leader create, and require it in the RPC.

ALTER TABLE public.return_leader_handovers
  ADD COLUMN IF NOT EXISTS agent_signature_url text,
  ADD COLUMN IF NOT EXISTS agent_signature_path text;

DROP FUNCTION IF EXISTS public.create_return_leader_handover(jsonb, text, jsonb);
