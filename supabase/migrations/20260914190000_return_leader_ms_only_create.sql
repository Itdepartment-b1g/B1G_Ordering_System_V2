-- Only mobile sales (agents) can create Return to Leader handovers.
-- Team leaders approve/receive; they no longer initiate RL.

CREATE OR REPLACE FUNCTION public.create_return_leader_handover(
  p_items jsonb,
  p_notes text DEFAULT NULL,
  p_attachments jsonb DEFAULT NULL,
  p_agent_signature_url text DEFAULT NULL,
  p_agent_signature_path text DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_role NOT IN ('mobile_sales', 'sales_agent') THEN
    RETURN json_build_object('success', false, 'error', 'Only mobile sales can submit a return to leader');
  END IF;
  ...
  v_sig_url := NULLIF(btrim(COALESCE(p_agent_signature_url, '')), '');
  v_sig_path := NULLIF(btrim(COALESCE(p_agent_signature_path, '')), '');
  IF v_sig_url IS NULL OR v_sig_path IS NULL THEN
    RETURN json_build_object('success', false, 'error', 'Agent signature is required');
  END IF;
...
GRANT EXECUTE ON FUNCTION public.create_return_leader_handover(jsonb, text, jsonb, text, text) TO authenticated;
