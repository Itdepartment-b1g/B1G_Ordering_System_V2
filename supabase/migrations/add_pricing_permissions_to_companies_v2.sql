-- ============================================================================
-- ADD PRICING PERMISSIONS TO COMPANIES (V2 - Fixed)
-- ============================================================================
...
DO $$ 
BEGIN
    -- Add team_leader_allowed_pricing if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'companies' 
        AND column_name = 'team_leader_allowed_pricing'
    ) THEN
        ALTER TABLE companies
        ADD COLUMN team_leader_allowed_pricing JSONB DEFAULT '["selling_price", "dsp_price", "rsp_price"]'::jsonb;
    END IF;

    -- Add mobile_sales_allowed_pricing if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'companies' 
        AND column_name = 'mobile_sales_allowed_pricing'
    ) THEN
        ALTER TABLE companies
        ADD COLUMN mobile_sales_allowed_pricing JSONB DEFAULT '["rsp_price"]'::jsonb;
    END IF;
END $$;
...
ALTER TABLE companies
ALTER COLUMN team_leader_allowed_pricing SET DEFAULT '["selling_price", "dsp_price", "rsp_price"]'::jsonb,
ALTER COLUMN team_leader_allowed_pricing SET NOT NULL;

ALTER TABLE companies
ALTER COLUMN mobile_sales_allowed_pricing SET DEFAULT '["rsp_price"]'::jsonb,
