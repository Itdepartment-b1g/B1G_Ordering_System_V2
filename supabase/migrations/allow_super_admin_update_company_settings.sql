-- ============================================================================
-- SECURE FUNCTION TO UPDATE ONLY PRICING PERMISSIONS
-- ============================================================================
...
BEGIN
    ...
    IF NOT (
        ...
        p_mobile_sales_pricing <@ '["selling_price", "dsp_price", "rsp_price"]'::jsonb AND
        jsonb_array_length(p_team_leader_pricing) > 0 AND
        jsonb_array_length(p_mobile_sales_pricing) > 0
    ) THEN
        RETURN json_build_object(
            'success', false, 
            'message', 'Invalid pricing columns. Must be non-empty arrays containing only: selling_price, dsp_price, rsp_price'
        );
    END IF;

    -- 5. Update ONLY the two pricing columns for the user's company
    UPDATE companies
    SET 
...
GRANT EXECUTE ON FUNCTION update_company_pricing_permissions TO authenticated;
